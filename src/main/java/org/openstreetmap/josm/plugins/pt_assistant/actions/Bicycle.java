 ...
 * @author Ashish Singh
 */
public class Bicycle extends MendRelationAction{
...
String getQuery() {
    ...
    if (super.aroundStops) {
        nodeList.addAll(super.members.stream().filter(RelationMember::isNode).map(RelationMember::getNode).collect(Collectors.toList()));
    }
...
public void callNextWay(int idx){
    ...
    Node node = checkVaildityOfWays(way,nexidx);
		...
		if(super.noLinkToPreviousWay) {
			...
			if(node==null) {
        if(super.members.get(super.currentIndex).getRole().equals("forward") && prelink.isOnewayLoopBackwardPart){
          node = way.lastNode();
        }
        else if(super.members.get(super.currentIndex).getRole().equals("backward") && prelink.isOnewayLoopForwardPart){
          node = way.lastNode();
        }
        else {
...
boolean checkOneWaySatisfiability(Way way, Node node) {
    ...
    if((link.isOnewayLoopBackwardPart && super.relation.hasTag("route", "bicycle"))||prelink.isOnewayLoopBackwardPart){
              return true;
            }
...
List<Way> removeInvalidWaysFromParentWays(List<Way> parentWays, Node node, Way way) {
    ...
    for (Way w : parentWays) {
        ...
        } else if (w.getNodes().contains(super.nextWay.lastNode()) && !w.isFirstLastNode(super.nextWay.lastNode())) {
    ...
    if (ways != null) {
        ...
        for (int i = 0; i < ways.size(); i++) {
            ...
            if (w.isNew()) {
             ...
             // System.out.println("printing current index: " + super.currentIndex + "and after what index member will be added "+ ind);
                addNewWays(Collections.singletonList(w), ind);
                prev = w;
                ind++;
        ...
        // System.out.println("current Way: " + way.getUniqueId() + "super.nextWay: "+nextw.getUniqueId()+ "super.currentNode: "+super.currentNode.getUniqueId());

        // System.out.println("current Way: " + way.getUniqueId() + "super.nextWay: "+super.nextWay.getUniqueId()+ "super.currentNode: "+super.currentNode.getUniqueId());
        if(n !=null){
        ...
        // System.out.println("check that have you traversed common node or not "+ super.currentNode.getUniqueId() + "value: "+Isthere.get(super.currentNode));
        if(Isthere.get(super.currentNode)!=null && Isthere.get(super.currentNode)>=3){
          super.currentIndex++;
          if(super.currentIndex <= super.members.size()-1){
            // System.out.println("super.currentNode will be "+ super.currentNode.getUniqueId());
...
void addNewWays(List<Way> ways, int i) {
    try {
        ...
        super.currentNode = getOtherNode(w,super.currentNode);
         // System.out.println("super.currentIndex is "+ super.currentIndex +" Way id is "+ super.members.get(super.currentIndex).getWay().getUniqueId());
        links = connectionTypeCalculator.updateLinks(super.members);
    } catch (Exception e) {
        Logging.error(e);
    }
...
String assignRoles(Way w){
  ...
  // System.out.println("Current Node: "+super.currentNode.getUniqueId());
